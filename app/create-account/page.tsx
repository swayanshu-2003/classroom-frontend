...
const page = () => {
    ...
    const onSubmit = async (data: any) => {
        ...
        const res: any = await axios.post(`${process.env.NEXT_PUBLIC_API_BASE_URL}/user/create`, data)
                        ...
                        <div className="mt-12 flex flex-col items-center">
                            ...
                            <div className="w-full flex-1 ">
                                ...
                                <form className="mx-auto max-w-xs" onSubmit={handleSubmit(onSubmit)}>
                                    <input
                                        className="w-full px-8 py-4 my-5 rounded-lg font-medium bg-gray-100 border border-gray-200 placeholder-gray-500 text-sm focus:outline-none focus:border-gray-400 focus:bg-white"
                                        type="text" placeholder="Full Name"
                                        {...register("name", { required: "name is required" })}
                                    />
                                    {errors?.name && <small className="text-red-500 text-xs">{errors.name.message?.toString()}</small>}

                                    <input
                                        className="w-full px-8 py-4 rounded-lg font-medium bg-gray-100 border border-gray-200 placeholder-gray-500 text-sm focus:outline-none focus:border-gray-400 focus:bg-white"
                                        type="email" placeholder="Email" {...register("username", { required: "username is required" })} />
                                    ...
                                    <div className="my-6 border-b text-center">
                                        <Link href="/login"
                                            className="leading-none px-2 inline-block text-sm text-blue-600 tracking-wide font-medium bg-white transform translate-y-1/2">
